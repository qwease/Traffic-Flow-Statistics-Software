        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                if trk:
                    ...
                    prev_set_len=len(seen_set)
                    # print(prev_set_len)
                    if len(tracked_dets)>0:
                        identities = tracked_dets[:, 8]
                        # print(identities[0])
                        seen_set.add(identities[0])
                    # print(len(seen_set))
                    # print(prev_set_len<len(seen_set))
                    for track in tracks:#different object is assigned with different track
                        ...
                        if is_vehicle(det_cls):
                            if centroid_vector(track.centroids)>cross_length and (up_interseted_1 == True or up_interseted_2 ==True):
                                vehicle_up_count+=1
                            elif centroid_vector(track.centroids)<-cross_length and (down_interseted_1==True or down_interseted_2==True):
