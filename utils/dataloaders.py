...
class LoadImages:
    ...
    def __next__(self):
        if self.terminate_cap==True or self.count == self.nf:
            raise StopIteration
...
class LoadStreams:
    ...
    def update(self, i, cap, stream):
        ...
        while cap.isOpened() and n < f:
            ...
            time.sleep(1/self.fps[-1])  # wait time
    ...
    def __next__(self):
        self.count += 1
        if self.terminate_cap or not all(x.is_alive() for x in self.threads) or cv2.waitKey(1) == ord('q'):  # q to quit
            cv2.destroyAllWindows()
            raise StopIteration
